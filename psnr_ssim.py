...
psnr_ls, ssim_ls, average_psnr, average_ssim = [], [], [], []
...
for image_name in image_filenames:
    ...
    img_out = np.array(out[0].permute(1, 2, 0).cpu().detach())
    img_gt = np.array(img_gt)
    img_out = np.array(image_pil)
...
average_psnr.append(sum(psnr_ls) / len(psnr_ls))
average_ssim.append(sum(ssim_ls) / len(ssim_ls))

dictionary = {"Model": [model_path], "PSNR": [average_psnr], "SSIM": [average_ssim]}
